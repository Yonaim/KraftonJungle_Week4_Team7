...
void FMaterialManager::LoadAllMaterials(ID3D11Device* InDevice, CRenderStateManager* InStateManager)
{
    ...
    auto MaterialDir = FPaths::ShaderCacheDir(); // 임시 Dir 경로
    try
    {
        if (!fs::exists(MaterialDir) /* && fs::is_directory(FPaths::MaterialDir()) */)
        {
            UE_LOG("MaterialManager", ELogVerbosity::Warning, "Material dir not exists\n");
            return;
        }
    ...
    {
        UE_LOG("MaterialManager", ELogVerbosity::Error,
               "[MaterialManager] Filesystem Error while preload materials: %s\n", ex.what());
    }
}
    ...
    )
{
    ...
    if (Json.contains("VertexShader"))
    {
        ...
        FWString WVSPath = (FPaths::AppRoot() / VSRelPath).wstring();
        auto     VS = FShaderMap::Get().GetOrCreateVertexShader(InDevice, WVSPath, L"main");
    ...
    if (Json.contains("PixelShader"))
    {
        ...
        std::wstring WPSPath = (FPaths::AppRoot() / PSRelPath).wstring();
        auto         PS = FShaderMap::Get().GetOrCreatePixelShader(InDevice, WPSPath, L"main");
    ...
    if (Json.contains("RenderState"))
    {
        ...
        {
            depthStencilOption.DepthWriteMask = RenderStatesJson["DepthWrite"].get<
    ...
    if (Json.contains("ConstantBuffers"))
    {
        for (auto& CBJson : Json["ConstantBuffers"])
        {
            ...
            for (auto& Info : ParamList)
            {
                ...
                else if (Info.Type == "float4x4" && Info.Value.is_array() && Info.Value.size() >=
                         16)
                {
                    float Val[16];
