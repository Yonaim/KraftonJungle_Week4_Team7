...
bool CRenderer::Initialize(HWND InHwnd, int32 Width, int32 Height)
{
	...
	std::wstring ShaderDirW = FPaths::ShaderCacheDir();     // 임시 Dir 경로
	std::wstring VSPath = ShaderDirW + L"NewShaderMesh.hlsl";
	std::wstring PSPath = ShaderDirW + L"NewShaderMesh.hlsl";

	if (!ShaderManager.LoadVertexShader(Device, VSPath, L"main")) return false;
	if (!ShaderManager.LoadPixelShader(Device, PSPath, L"main")) return false;

	/** 기본 Material 생성 */
	{
		auto VS = FShaderMap::Get().GetOrCreateVertexShader(Device, VSPath, L"main");       // EntryPoint는 임시값
		std::wstring ColorPSPath = ShaderDirW + L"ColorPixelShader.hlsl";
		auto PS = FShaderMap::Get().GetOrCreatePixelShader(Device, ColorPSPath, L"main");    // EntryPoint는 임시값
	...
	/** Texture 용 Material 생성 */
	{
		...
		auto PS = FShaderMap::Get().GetOrCreatePixelShader(Device, TexturePSPath, L"main");
...
void CRenderer::ExecuteRenderPass(ERenderLayer InRenderLayer)
{
	...
	ID3D11SamplerState* FontSampler = NormalSampler;
...
bool CRenderer::InitOutlineResources()
{
	...
	std::filesystem::path PSPath = (FPaths::ShaderCacheDir() / "OutlinePixelShader.hlsl");
	OutlinePS = FShaderMap::Get().GetOrCreatePixelShader(Device, PSPath, L"main");
