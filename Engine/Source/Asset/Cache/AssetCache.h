...
template <typename TData> class TAssetCache
{
  public:
    ...
    template <typename TKey> std::shared_ptr<TData> Find(const TKey& Key) const
    {
        static_assert(std::is_base_of_v<FAssetKey, TKey>, "TKey must derive from FAssetKey.");
        return Find(static_cast<const FAssetKey&>(Key));
    }

    void Insert(const FAssetKey& Key, const std::shared_ptr<TData>& Data) { Records[Key] = Data; }

    template <typename TKey> void Insert(const TKey& Key, const std::shared_ptr<TData>& Data)
    {
        static_assert(std::is_base_of_v<FAssetKey, TKey>, "TKey must derive from FAssetKey.");
    ...
    template <typename TKey> void Remove(const TKey& Key)
    {
        static_assert(std::is_base_of_v<FAssetKey, TKey>, "TKey must derive from FAssetKey.");
...
template <typename TIntermediate> using TIntermediateCache = TAssetCache<TIntermediate>;

template <typename TCooked> using TCookedCache = TAssetCache<TCooked>;
