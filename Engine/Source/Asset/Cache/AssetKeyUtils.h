...
namespace KeyUtils
{
    ...
    {
        KeyHash::Combine(Seed, V.X); KeyHash::Combine(Seed, V.Y); KeyHash::Combine(Seed, V.Z);
    }
    inline void HashVector2(size_t& Seed, const FVector2& V)
    {
        KeyHash::Combine(Seed, V.X); KeyHash::Combine(Seed, V.Y);
    }
    ...
    inline FTextureIntermediateKey MakeIntermediateKey(const FIntermediateTextureData& Data)
    {
        ...
        for (uint8 Byte : Data.Pixels) KeyHash::Combine(Seed, Byte);
    ...
    inline FSubUVAtlasIntermediateKey MakeIntermediateKey(const FIntermediateSubUVAtlasData& Data)
    {
        size_t Seed = 0;
        ...
        {
            KeyHash::Combine(Seed, Frame.Id); KeyHash::Combine(Seed, Frame.X); KeyHash::Combine(Seed, Frame.Y);
            KeyHash::Combine(Seed, Frame.Width); KeyHash::Combine(Seed, Frame.Height);
            KeyHash::Combine(Seed, Frame.PivotX); KeyHash::Combine(Seed, Frame.PivotY); KeyHash::Combine(Seed, Frame.Duration);
        }
    ...
    inline FFontAtlasIntermediateKey MakeIntermediateKey(const FIntermediateFontAtlasData& Data)
    {
        ...
        for (const auto& Pair : Data.Glyphs)
        {
            ...
            KeyHash::Combine(Seed, G.Id); KeyHash::Combine(Seed, G.X); KeyHash::Combine(Seed, G.Y);
            KeyHash::Combine(Seed, G.Width); KeyHash::Combine(Seed, G.Height);
            KeyHash::Combine(Seed, G.XOffset); KeyHash::Combine(Seed, G.YOffset); KeyHash::Combine(Seed, G.XAdvance);
            KeyHash::Combine(Seed, G.Page); KeyHash::Combine(Seed, G.Channel);
        }
        return {FinalizeHash(Seed)};
    }
