...
{
    return FMatrix::RotationXMatrix(Roll) * FMatrix::RotationYMatrix(Pitch) *
...
FRotator FRotator::FromVector(const FVector &Forward)
{
    ...
    float Pitch = MathHelper::Atan2(
                      Forward.z, MathHelper::Sqrt(Forward.x * Forward.x + Forward.y * Forward.y)) *
